...
class ReasonTable extends Component{
  ...
  render(){
    ...
    return(
        <div>
          {hasAction(btn,'Notice_addnotice')&& <Form onOk={this.getData} />}
          <Table dataSource={data} hasBorder={false} >
            ...
            <Table.Column
              ...
              cell={(text,index,record)=>(
                <Switch checked={text===1} disabled={!hasAction(btn,'Notice_updatenotice')} onChange={()=>this.update({id:record.id,status:text===1?0:1})} />
              )}
            />
            ...
            <Table.Column
              ...
              cell={(text,index,record)=> (
                <div>
                  {hasAction(btn,'Notice_updatenotice')&&<Revise data={record}  onOk={this.getData}/>}
                  {hasAction(btn,'Notice_delnotice')&&<Button  type="normal" shape="text"  onClick={()=>this.deleteData(record.id)}>{_intl.get('public.delete')}</Button>}
                </div>
              )}
            />
          </Table>
