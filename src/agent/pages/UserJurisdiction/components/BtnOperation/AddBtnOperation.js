    ...
    return (
      <span>
        <Button onClick={this.onOpen} style={{marginRight:'15px'}}>
          <Icon type="add" />{_intl.get('userJurisdiction.addGroups')}
        </Button>
        <Dialog
        ...
        >
          ...
          <Row style={styles.formItem}>
            ...
            {_intl.get('userJurisdiction.right')}：
            </Col>
            <Col xxs="16" s="10" l="6">
              <IceFormBinder
                name="operations"
              ...
              >
              <Select multiple defaultValue={btnOperationArr}  onChange={this.handleChange} dataSource={dataSource} style={{width: 300}} />
              </IceFormBinder>
          ...
          <Row>
            <Col xxs="6" s="4" l="3" style={styles.formLabel}>
            {_intl.get('userJurisdiction.name')}：
            </Col>
            <Col xxs="16" s="10" l="6">
              <IceFormBinder
                name="name"
                rules={[
                  {required:true,message:_intl.get('public.required')},
                  {min:1,max:10,message:_intl.get('userJurisdiction.minOneMaxTen')}
                ]}
              >
              <Input defaultValue={btnName}  style={{width: 300}} placeholder={_intl.get('userJurisdiction.pleaseRightName')}/>
              </IceFormBinder>
